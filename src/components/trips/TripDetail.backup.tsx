...
const TripDetail = () => {
  ...
  const handleUpdateLRCharge = async () => {
    ...
    try {
      ...
      // Instead of trying to update lrCharges directly (which may not be supported by the API),
      // we'll update the trip locally and then recalculate the balance payment
      const updatedTripData = {
        ...trip,
        lrCharges: lrChargeValue
      };
      
      // Update local state immediately to show changes in the UI
      ...
      // Calculate total deductions (deduction charges + LR charges + platform fees)
      const deductionChargesTotal = updatedTripData.deductionCharges?.reduce((sum, charge) => sum + charge.amount, 0) || 0;
      const platformFees = updatedTripData.platformFees || 250;
      const totalDeductions = deductionChargesTotal + lrChargeValue + platformFees;
      
      // Calculate the new balance payment (original supplier freight - advance - total deductions)
      const originalBalanceAmount = updatedTripData.supplierFreight - updatedTripData.advanceSupplierFreight;
      const adjustedBalanceAmount = Math.max(0, originalBalanceAmount - totalDeductions);
      ...
      // Only update the balanceSupplierFreight field in the API
      if (roundedAmount !== updatedTripData.balanceSupplierFreight) {
        try {
          // Only update balanceSupplierFreight directly
          const payload = { balanceSupplierFreight: roundedAmount };
          console.log("Sending balance update payload:", JSON.stringify(payload));
          
          const response = await api.trips.update(trip.id, payload);
          console.log("Balance update successful:", response);
          
          // Update the local state with the response data to ensure accuracy
          setTrip({
            ...updatedTripData,
            balanceSupplierFreight: roundedAmount
          });
          ...
          // Emit event to notify payment dashboard and other components
          events.emit(EVENT_TYPES.PAYMENT_STATUS_CHANGED, {
            tripId: updatedTripData.id,
            paymentType: 'balance',
            oldStatus: updatedTripData.balancePaymentStatus,
            newStatus: updatedTripData.balancePaymentStatus,
            amountChanged: true,
            oldAmount: updatedTripData.balanceSupplierFreight,
            newAmount: roundedAmount,
            timestamp: Date.now()
          });
        } catch (updateError) {
          ...
          // Leave the local UI updated but show a warning
          toast({
            title: "Partial Update",
            description: "LR charges were updated in the UI, but the balance calculation couldn't be saved to the server.",
            variant: "default"
          });
      ...
      toast({
        ...
        description: "There was an error updating the charges. Please try again.",
        variant: "destructive"
      });
    }
  };
  ...
  const handleUpdatePlatformFee = async () => {
    ...
    try {
      ...
      // Instead of trying to update platformFees directly (which may not be supported by the API),
      // we'll update the trip locally and then recalculate the balance payment
      const updatedTripData = {
        ...trip,
      ...
      // Calculate total deductions (deduction charges + LR charges + platform fees)
      const deductionChargesTotal = updatedTripData.deductionCharges?.reduce((sum, charge) => sum + charge.amount, 0) || 0;
      const lrCharges = updatedTripData.lrCharges || 250;
      ...
      if (roundedAmount !== updatedTripData.balanceSupplierFreight) {
        try {
          ...
          toast({
            title: "Partial Update",
            description: "Platform fees were updated in the UI, but the balance calculation couldn't be saved to the server.",
            variant: "default"
          });
      ...
      toast({
        ...
        description: `Platform fees updated to ₹${platformFeeValue.toLocaleString()}`,
      });
    } catch (error) {
    ...
    try {
      ...
      if (fileData.type.toLowerCase().includes("pod") || fileData.name.toLowerCase().includes("pod")) {
        ...
        try {
          ...
          let errorMsg = "The document was saved locally but server update failed. Changes will sync when connection is restored.";
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex-1">
                    ...
                    <FileActions
                      ...
                      existingFiles={getDocuments(`${trip.id}-lr`)}
                    />
              </div>
                  ...
                  <div className="flex-1">
                    ...
                    <FileActions
                      ...
                      onSuccess={handleDocumentUpload}
                      existingFiles={getDocuments(`${trip.id}-invoice`)}
                    />
              </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex-1">
                    ...
                    <FileActions
                      ...
                      type="eway-document"
                      entityName={trip.orderNumber}
                      documentType="E-waybill"
                      onSuccess={handleDocumentUpload}
                      existingFiles={getDocuments(`${trip.id}-eway`)}
                    />
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <FileActions
                      ...
                      documentType="POD"
                      onSuccess={handleDocumentUpload}
                      existingFiles={getDocuments(`${trip.id}-pod`)}
                    />
                  </div>
                </div>
              </div>
